...
export default function Dashboard() {
  ...
  const { metrics, providerHealth, connected } =
    useRealtimeMetrics(selectedProjectId);
    ...
    if (selectedProjectId) {
      (async () => {
        ...
        if (import.meta.env.DEV) {
          console.log(
            "[Dashboard] Fetching alerts events, token present:",
            !!session,
          );
        }
        try {
          const res = await fetch(
            `${API_URL}/api/alerts/events?project_id=${selectedProjectId}`,
            { headers },
          );
          if (res.ok) {
            const events = await res.json();
            setAlerts(events?.filter((e) => e.status === "triggered") || []);
          }
        } catch (err) {
          console.warn("[Dashboard] Failed to fetch alerts:", err.message);
        }
      })();
    }
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400">
              Real-time LLM inference metrics
            </p>
          </div>
          <div className="flex items-center gap-2">
            <span
              className={`inline-block h-2 w-2 rounded-full ${connected ? "bg-green-400" : "bg-amber-400"}`}
            />
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="mt-1 space-y-1">
            {alerts.slice(0, 3).map((a) => (
              <div key={a.id} className="text-xs text-red-300">
                {a.alerts?.name || "Alert"} — Value: {a.triggered_value} —{" "}
      ...
      {providerHealth.length > 0 && (
        <div className="flex flex-wrap gap-3">
          {providerHealth.map((p) => (
            <div
              key={p.provider}
              className="flex items-center gap-2 rounded-xl border border-white/10 bg-[#1e293b] px-3 py-2"
            >
              ...
              <span
                className={`text-xs ${p.success_rate > 95 ? "text-green-400" : p.success_rate > 80 ? "text-amber-400" : "text-red-400"}`}
              >
                {p.success_rate}%
              </span>
            </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <MetricCard
          title="Successful"
          value={summary.successful_requests || 0}
          unit=""
          icon="✅"
        />
        <MetricCard
          title="Failed"
          value={summary.failed_requests || 0}
          unit=""
          icon="❌"
        />
        <MetricCard
        ...
        <MetricCard
          title="Req/min"
          value={summary.requests_per_minute || 0}
          unit=""
          icon="📈"
        />
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <MetricCard
          title="Est. Cost"
          value={formatCost(costData?.estimated_total)}
          unit=""
          icon="💰"
        />
        <MetricCard
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {models.map((m) => (
                  <tr
                    key={m.name}
                    className="border-b border-white/5 transition-colors hover:bg-[#1e293b]/50"
                  >
