...
const ALERT_TYPES = [
  {
    id: "latency_spike",
    label: "Latency Spike",
    desc: "Average latency exceeds threshold",
  },
  {
    id: "error_rate_spike",
    label: "Error Rate Spike",
    desc: "Error rate exceeds threshold (%)",
  },
  {
    id: "token_burn_spike",
    label: "Token Burn Spike",
    desc: "Token usage exceeds threshold",
  },
  {
    id: "provider_outage",
    label: "Provider Outage",
    desc: "Provider failures exceed threshold",
  },
  {
    id: "throughput_drop",
    label: "Throughput Drop",
    desc: "Requests/min drops below threshold",
  },
];
...
export default function Alerts() {
  ...
  const { data: events } = useApi(
    selectedProjectId ? `/api/alerts/events${params}` : "",
  );
  ...
  const createAlert = async () => {
    ...
    try {
      ...
      if (import.meta.env.DEV) console.log("[Alerts] POST", url);
      ...
      if (!res.ok) console.error("[Alerts] create failed:", url, res.status);
      setShowCreate(false);
      setForm({
        name: "",
        alert_type: "latency_spike",
  ...
  const deleteAlert = async (id) => {
    if (!id) {
      console.warn("[Alerts] deleteAlert: no id");
      return;
    }
    ...
    try {
      ...
      if (import.meta.env.DEV) console.log("[Alerts] DELETE", url);
      const res = await fetch(url, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${session.access_token}` },
      });
      if (!res.ok) console.error("[Alerts] delete failed:", url, res.status);
      refetch();
    } catch (err) {
  ...
  const toggleAlert = async (alert) => {
    if (!alert?.id) {
      console.warn("[Alerts] toggleAlert: no alert.id");
      return;
    }
    const url = `${API_URL}/api/alerts/${alert.id}`;
    try {
      const {
        data: { session },
      } = await supabase.auth.getSession();
      if (import.meta.env.DEV) console.log("[Alerts] PATCH", url);
      const res = await fetch(url, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
      ...
      if (!res.ok) console.error("[Alerts] toggle failed:", url, res.status);
      refetch();
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400">
            Configure observability alerting rules
          </p>
        </div>
        <button
      ...
      {showCreate && selectedProjectId && (
        <div className="rounded-2xl border border-white/10 bg-[#1e293b] p-5 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, notification_channel: e.target.value })
                }
                className="w-full rounded-lg border border-white/10 bg-[#0f172a] px-3 py-2 text-sm text-white focus:border-blue-500 focus:outline-none"
              >
          ...
          <button
            onClick={createAlert}
            disabled={!form.name || !form.threshold_value}
      ...
      {loading && (
        <div className="h-32 animate-pulse rounded-2xl bg-[#1e293b]" />
      )}
      ...
      {alerts && alerts.length > 0 && (
        <div className="space-y-3">
          {alerts.map((alert) => (
            <div
              key={alert.id}
              className={`rounded-2xl border p-4 transition-colors ${alert.is_active ? "border-white/10 bg-[#1e293b]" : "border-white/5 bg-[#1e293b]/50 opacity-60"}`}
            >
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    <span
                      className={`h-2 w-2 rounded-full ${alert.is_active ? "bg-green-400" : "bg-gray-600"}`}
                    />
                    ...
                    <span className="rounded-full bg-[#0f172a] px-2 py-0.5 text-xs text-gray-400">
                      {ALERT_TYPES.find((t) => t.id === alert.alert_type)
                        ?.label || alert.alert_type}
                    </span>
                  </div>
                  <p className="mt-1 text-xs text-gray-500">
                    Threshold: {alert.comparison_operator}{" "}
                    {alert.threshold_value} · Window:{" "}
                    {alert.time_window_minutes}m · Channel:{" "}
        ...
        <div className="rounded-2xl border border-white/10 bg-[#1e293b] p-5">
          ...
          <div className="space-y-2">
            {events.slice(0, 10).map((evt) => (
              <div
                key={evt.id}
                className="flex items-center justify-between rounded-lg bg-[#0f172a] px-3 py-2"
              >
                <div>
                  <span className="text-sm text-white">
                    {evt.alerts?.name || "Alert"}
                  </span>
                  <span className="ml-2 text-xs text-gray-500">
                    Value: {evt.triggered_value}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs font-medium ${evt.status === "triggered" ? "bg-red-900/30 text-red-400" : "bg-green-900/30 text-green-400"}`}
                  >
                    {evt.status}
                  </span>
